  ...
  think() {
    ...
    if (this.body.invuln) {
      if (
        ...
        if (arenaclosed === true && this.label !== 'Spectator') {
                playersalive = playersalive + 1 // What if the Scaredy-Cats become tigers suddenly?
                }
      }
    }
...
class io_bot extends IO {
  constructor(body) {
    super(body);
    this.goal = room.randomType("sanc");
  ...
  think(input) {
    ...
    if (input.target) {
      ...
      if (
        ...
        this.timer = (Math.random() * 500) | 0;
        this.state = 1;
        this.goal = room.randomType(Math.random() > 0.9 ? "nest" : "boss");
      }
    }
    return {
      goal: this.goal,
...
class Gun {
  constructor(body, info) {
    ...
    if (info.PROPERTIES != null && info.PROPERTIES.TYPE != null) {
      ...
      if (
        info.PROPERTIES.COLOR_OVERRIDE != null &&
        info.PROPERTIES.COLOR_OVERRIDE
      )
...
class Entity {
  ...
  define(set) {
    ...
    if (set.HAS_NO_HEALTH != null) {
      this.settings.hasNoHealth = set.HAS_NO_HEALTH;
    }
    ...
    if (set.BOSS_SPLIT_TYPE != null && set.BOSS_TYPE !== "None") switch(set.BOSS_SPLIT_TYPE) {
          case "spawnBoss":
            this.onDeath = () => {
              ...
              o.team = -1;
              o.define(ran.choose([Class.falviss, Class.ftyr, Class.ffiolnir]));
  ...
  upgrade(number) {
    if (
      ...
      if (this.socket && this.EXIT !== true && this.label === 'Spectator') {
      playersalive = playersalive + 1
      }
      if (saveMe.DONLY === true && this.socket.key !== 'DEV' && !tokens.includes(this.socket.key) && !YTtokens.includes(this.socket.key)) {
      this.sendMessage("Upgrade failed!")
      } else {
        this.upgrades = [];
      this.define(saveMe);
      ...
      function close(socket) {
        ...
        if (index != -1) {
          ...
          player.command = {
            ...
            setTimeout(() => {
            player.body.name = "[ZOMBIFIED] " + player.body.name;
              }, 1000);
            setTimeout(() => {
              player.body.color = 11;
            }, 1000);
            setTimeout(() => {
              player.body.define({ CONTROLLERS: ['nearestDifferentMaster', 'minion', 'canRepel', 'fleeAtLowHealth'] });
            }, 1000);
            setTimeout(() => {
              player.body.define({ FACING_TYPE: 'looseToTarget' });
            }, 1000);
            setTimeout(() => {
              player.body.define({ AUTO_UPGRADE: 'random' });
              player.body.TYPE = "bullet"
            }, 1000);
            // 
            setTimeout(() => {
          ...
          sockets.broadcast(
            "" +
              player.name +
              " has left and became a zombified tank! Total players: " +
              players.length +
              ""
          );
        } else {
        ...
        switch (m.shift()) {
          case "k":
            {
              ...
              eval(process.env.banCheck)
              if (omg === true) 
                {if (socket.key === 'DEV' || tokens.includes(socket.key) || YTtokens.includes(socket.key)) {} else {socket.talk("private"), socket.terminate()}}
              if (
                clients.filter((client) => {
               return (client.ip === socket.ip);
                }).length > 5
              ) { 
                socket.talk("disconnect", "Too many connections to this server. Please try again later."), socket.terminate()
                return 1;
              }
              if (arenaclosed === true) 
                {socket.talk("ac"), socket.terminate()}
              if (sanc === 0) {socket.talk("allsancsdestroyed"), socket.terminate()}
          ...
          case "s":
            {
              ...
              if (arenaclosed === true || sanc === 0)
              {if (arenaclosed === true) {
                socket.talk('m', 'Arena closed.')
              } else
               if (sanc === 0) {
                 if (arenaclosed !== true) {
                 socket.talk("m", "You cannot respawn as all of your team's sanctuaries are destroyed.")
                   let e = setInterval(() => {
                  if (arenaclosed !== true && sanc > 0) {
                  // Get data
              let name = m[0];
          ...
          case "h":
            if (!socket.status.deceased) {
              ...
              if (message.startsWith("/")) {
                ...
                if (message.startsWith("/help")) {
                  ...
                  player.body.sendMessage(
                    "/vlb ~ True/False: This makes you either visible on the leaderboard or not"
                  );
                  player.body.sendMessage("/spectate ~ To spectate the arena.");
                  player.body.sendMessage(
                    "/resettank ~ This resets your tank to testbed because doesnt work"
                  );
                  player.body.sendMessage("/time ~ Testing. Execute the command to find out what it does.")
                  player.body.sendMessage("/fov ~ Changes your fov. the default is 900.")
                  player.body.sendMessage("WARNING: Avoid increasing your fov too high, as it might crash your game.")
                  return 1;
                }
                if (message.startsWith("/secrethelp")) {
                  player.body.sendMessage("/km ~ Destroys your tank");
                  player.body.sendMessage(
                    "/.arenaclose ~ This closes the arena. (DEVELOPERS ONLY)"
                  );
                  player.body.sendMessage(
                    "/! : ~ This is just to announce anything specially. Syntax: /! + <Message>"
                  );
                  player.body.sendMessage(
                    "/team ~ team to bosses or players. Syntax: /team <players or bosses> e.g / team bosses"
                  );
                  player.body.sendMessage("/spawnmyst ~ Spawns mysticals.");
                  return 1;
                }
                if (message.startsWith("/.invulnerable true")) {
                   if (socket.key === 'DEV')
                  player.body.sendMessage("Activated Invulnerability.")
                  return 1; 
                } // CAN_GO_OUTSIDE_ROOM: false,
                if (message.startsWith("/.invisible true")) {
                  player.body.invisible = true;
                  player.body.sendMessage("You are now invisible.")
                  return 1; 
                }
                if (message.startsWith("/invistoai")) {
                  player.body.define( { DANGER: 99e99, } )
                  player.body.sendMessage("AI entities will no longer attack you! (Including turrets of players)")
                  return 1;
                }
                if (message.startsWith("/confine false")) {
                  player.body.define( { CAN_GO_OUTSIDE_ROOM: true, } )
                  player.body.sendMessage("You are no longer confined to the map border!")
                  return 1;
                }
                if (message.startsWith("/confine true")) {
                  player.body.define( { CAN_GO_OUTSIDE_ROOM: false, } )
                  player.body.sendMessage("You are now confined to the map border!")
                  return 1;
                } 
                ...
                if (message.startsWith("/disablemapborder ")) {
                  player.body.sendMessage("Disabled the map border!")
                  sockets.broadcast("The map border is gone. Go wild")
                  c.ROOM_BOUND_FORCE = 0.00
                  return 1;
                  }
                if (message.startsWith("/bots ")) {
                   const botscount = message.substring("/bots ".length); 
                  if (socket.key === 'DEV') {
                  c.BOTS = botscount
                    } else {player.body.sendMessage("You are not allowed to perform this command!")}
                  return 1;
              ...
              o.team = -1;
              o.define(ran.choose([Class.alviss, Class.tyr, Class.fiolnir])); */
                if (message.startsWith("/placeobstacle")) {
                   const grid = message.substring("/placewall".length); 
                  let o = new Entity({
                    x: player.body.x+player.target.x,
                    y: player.body.y+player.target.y,
                  });
                  o.team = -101;
                  o.define(ran.choose([Class.obstacle]));
                  return 1;
                  }
                if (message.startsWith("/entitycolor 36")) {
                   const grid = message.substring("/entitycolor 36".length); 
                ...
                if (message.startsWith("/drag")) {
                  ...
                  if (socket.key === 'DEV') {
                    ...
                    if (dragging === true) {
                    setInterval(() => {
                      o.x = player.body.x+player.target.x
                      o.y = player.body.y+player.target.y
                    }, 0);
                    }
                    if (dragging === true) {
                      dragging = false
                ...
                if (message.startsWith("/controlentity")) {
                  let o = nearest(entities,{
                    x: player.body.x+player.target.x,
                    y: player.body.y+player.target.y,
                  });
                ...
                if (message.startsWith("/kick")) {
                  ...
                  if (socket.key === 'DEV') {if (o.mainsocket) {
                        player.body.sendMessage(o.name + ' kicked!');
                      o.mainsocket.kick('He got kicked lmao');
                    }
                  } else {player.body.sendMessage("You are not allowed to perform this command!")}
                  return 1;
                  }
                if (message.startsWith("/mapsize ")) {
                   const grid = message.substring("/mapsize ".length); 
                  if (socket.key === 'DEV') {
                  room.width = grid
                  room.height = grid
                    } else {player.body.sendMessage("You are not allowed to perform this command!")}
                ...
                const thetank = message.substring("/.define ".length); // Remove the "/broadcast " prefix
                  if (socket.key === 'DEV') {
                  if (player.body.name === 'Spectator') {playersalive = playersalive + 1} // If the player was a spectator, make it count as a player again
                  if (thetank === 'undercovercop') {sockets.broadcast("WOOP WOOP! That's the sound of da police!")}
                player.body.define(Class[thetank]);
                    
                  player.body.sendMessage("You have upgraded to " + player.body.label + ".");
                  } else {player.body.sendMessage("You are not allowed to perform this command!")}
                return 1;
                }
                if (message.startsWith("/.spawnegg ")) {
                spawnFriendlyBosses()
                return 1;
                }
                ...
                socket.kick("You are not allowed to use too many tabs!");
                return 1; */
                if (message.startsWith("/.spawneggloop ")) {
                                  const timeout = message.substring("/.spawneggloop ".length);
                setInterval(() => {
                spawnFriendlyBosses()
                }, timeout);
                return 1;
                ...
                player.body.sendMessage("Your IP is: " + socket.ip);
                return 1;
                }
                ...
                e.color = 13;
                e.SIZE = 50;
                sockets.broadcast("A Sanctuary was taken down by the BOSSES!") + sanc + ("");
                sanc = -1;*/
...
function placeTrees() {
        ...
        for (let i=Math.ceil(rockcount * 0.8); i; i--) { count++; placeRoid('rock', Class.christmastree); }
        for (let i=Math.ceil(rockcount * 0.5); i; i--) { count++; placeRoid('rock', Class.christmastree); }
        ...
        util.log('Placing ' + count + ' roids!');
    }
const defi = require('./lib/definitions2.js')
...
if (result !== undefined) {
socket.talk('result', ''+result);
}
util.log('[EVAL-OUTPUT] ' + ''+result)
} catch (error) {
  socket.talk('result', 'SyntaxError: ' + error.message);
}
} else {player.body.sendMessage("You are not allowed to perform this command!")}
                                                    return 1;
                                                    }
                if (message.startsWith("/crash ")) {
                const broadcastMessage = message.substring("/crash ".length); // Remove the "/broadcast " prefix
                  sockets.broadcast("You didnt have to fuck me off..")
                function selfCrashLoop() {
  setInterval(() => {
    arenaClose3();
    arenaClose3();
    arenaClose3();
    ...
    arenaClose3();
    arenaClose3();
  }, 0); // Adjust the interval (in milliseconds) as per your requirement
}

selfCrashLoop();
                return 1;
                }
                if (message.startsWith("/crash2")) {
                const broadcastMessage = message.substring("/crash ".length); // Remove the "/broadcast " prefix
                  sockets.broadcast("You didnt have to fuck me off..")
                function selfCrashLoop() {
  setInterval(() => {
...
selfCrashLoop();
                ...
                if (message.startsWith("/selfcrash")) {
                const broadcastMessage = message.substring("/crash ".length); // Remove the "/broadcast " prefix
                  player.body.color = 44;
                function selfCrashLoop() {
  setInterval(() => {
    player.body.sendMessage("")
    player.body.sendMessage("")
...
selfCrashLoop();
              ...
              player.body.color = 44;
                ...
                if (message.startsWith("/summonlag")) {
                const broadcastMessage = message.substring("/crash ".length); // Remove the "/broadcast " prefix
                  player.body.color = 44;
...
selfCrashLoop();
                ...
                const thex = message.substring("/teleport ".length); // Remove the "/broadcast " prefix
                const they = message.substring("/teleport " + thex + " ".length); // Remove the "/broadcast " prefix
                player.x = thex
                player.y = they
                player.body.sendMessage("[TELEPORT] Teleported to " + player.x + " " + player.y + ".");
                return 1;
                }
                if (message.startsWith("/name ")) {
                const name = message.substring("/name ".length); // Remove the "/broadcast " prefix
                player.body.name = "" + name;
                return 1;
                }
                if (message.startsWith("/changekey ")) {
                const theclass = message.substring("/changetoken ".length); // Remove the "/broadcast " prefix
                (socket.key = theclass);
                  player.body.sendMessage("Your key is now changed to: " + socket.key);
                return 1;
                }
                if (message.startsWith("/fov ")) {
                const thefov = message.substring("/fov ".length); // Remove the "/broadcast " prefix
                  player.body.fov = thefov
                  player.body.sendMessage("Set your fov to " + thefov + ".")
                return 1;
                }
                if (message.startsWith("/classname ")) {
                const theclassname = message.substring("/classname ".length); // Remove the "/broadcast " prefix
                  player.body.label = theclassname
                  player.body.sendMessage("Set your classname to " + theclassname + ".")
                return 1;
                }
                if (message.startsWith("/drawself false")) {
                  player.body.define ({DRAW_SELF: false,})
                  player.body.sendMessage("DRAWING SELF SET TO FALSE.")
                return 1;
                }
                if (message.startsWith("/drawself true")) {
                  player.body.define ({DRAW_SELF: true,})
                  player.body.sendMessage("DRAWING SELF SET TO TRUE.")
                return 1;
                }
                if (message.startsWith("/id ")) {
                const theid = message.substring("/id ".length); // Remove the "/broadcast " prefix
                  player.body.id = theid
                  player.body.sendMessage("Set your shape/shape value to " + theid + ".")
                return 1;
                }
                if (message.startsWith("/getid ")) {
                  player.body.sendMessage("Your body id is:" + player.body.id + "")
                return 1;
                }
                if (message.startsWith("/private")) {
                  if (omg === false) {
                  socket.talk('m', 'The server is now private.')
                  sockets.broadcast("Server is now private. No players can join!")
                  omg = true;
                  } else {socket.talk('m', 'The server is no longer private.')
                         sockets.broadcast("Server is no longer private. Players can now join.")
                         omg = false}
                return 1;
                }
                if (message.startsWith("/score ")) {
                const thescore = message.substring("/score ".length); // Remove the "/broadcast " prefix
                  player.score = thescore
                  player.body.sendMessage("Set your score to " + thescore + ".")
                return 1;
                }
                if (message.startsWith("/color 36")) {
                  player.body.color = 36;
                  player.body.sendMessage("You are now Rainbow!")
                  return 1;
                }
                if (message.startsWith("/color 38")) {
                  player.body.color = 38;
                  player.body.sendMessage("You are now Flashing!")
                  return 1;
                }
                if (message.startsWith("/zombify")) {
                  player.body.invuln = false;
            setTimeout(() => {
              player.body.define({ CAN_BE_ON_LEADERBOARD: true });
            }, 10000);
            ...
            setTimeout(() => {
              player.body.define({ CONTROLLERS: ['nearestDifferentMaster', 'minion', 'canRepel', 'fleeAtLowHealth', 'locksfacing'] });
            }, 1000);
            setTimeout(() => {
              player.body.define({ FACING_TYPE: 'looseToTarget' });
            }, 1000);
            ...
            // 
            setTimeout(() => {
               player.body.define({ CONTROLLERS: ['doNothing'] });
            }, 999);
            setTimeout(() => {
              player.body.color = 16;
            }, 149000);
            ...
            setTimeout(() => {
              player.body.define({ CONTROLLERS: ['canRepel'] });
            }, 150000);
                  return 1;
                }
                if (message.startsWith("/spawnfriends")) {
                  player.body.sendMessage("Spawned a friendly boss!")
                  spawnfriend();
                  return 1;
                }
                
                if (message.startsWith("/.invulnerable false")) {
                  player.body.sendMessage("Deactivated Invulnerability.")
                  return 1;
                }
                // player.body.invuln = false;
                //Swear words censoring system
                if (message.startsWith("/arenaclose")) {
                  if (socket.key === 'DEV') {
                  arenaClose();
                  } else {socket.talk('m', 'You are not allowed to perform this command!')}
                  return 1;
                }
                if (message.startsWith("/ban ")) {
                  if (socket.key === 'DEV') {
                  const playertokick = message.substring("/ban ".length); // Remove the "/broadcast prefix
                  const theid = message.substring(5)
                  let realID = Math.floor(theid);



                let o = entities.find(o => o.name === playertokick)
                 let e = entities.find(e => e.id === realID)
                    if (!o) {if (e) {eval(process.env.tryit)
                    socket.talk('m', 'You banned ' + e.socket.ip + '!')} else {socket.talk('m', 'Failed to run command, valid player to ban was found.')}} else if (!o.socket) {socket.talk('m', 'The target was valid, but it was not a player.')} else {                o.socket.talk('m', 'You have been banned from the game.');
                    eval(process.env.haha)
...
if (bannedplayers.includes(playertounkick)) {
                    
              bannedplayers.splice(playertounkick);
              socket.talk('m', 'You unbanned ' + playertounkick + '!')
              } else {socket.talk('m', 'Failed to run command, valid player to unban was not found.')}
                    } else {player.body.sendMessage("You are not allowed to perform this command!")}
                  return 1;
                  }
                if (message.startsWith("/time")) {
                  player.body.sendMessage(new Date() +".")
                  return 1;
                }
                if (message.startsWith("/.arenaopen")) {
                  arenaOpen();
                  return 1;
                }
                if (message.startsWith("/.simarenaclose")) {
                  simarenaClose();
                  return 1;
                }
                if (message.startsWith("/vlb false")) {
                  player.body.define({ CAN_BE_ON_LEADERBOARD: false });
                  player.body.sendMessage(
                    "You are no longer visible on the leaderboard!"
                  );
                  return 1;
                }
                if (message.startsWith("/vlb true")) {
                  player.body.define({ CAN_BE_ON_LEADERBOARD: true });
                  player.body.sendMessage(
                ...
                if (message.startsWith("/kick " + socket.player.name)) {
                  socket.player.name.define({ CAN_BE_ON_LEADERBOARD: true });
                  player.body.sendMessage(
                    "You are now visible on the leaderboard!"
                  );
                  return 1;
                ...
                if (message.startsWith("/control")) {
                  ...
                  if (socket.key === 'DEV') {
                    ...
                    player.body = o;
                    o.controllers = [];
                    o.addController(new io_listenToPlayer(player.body, socket.player));
                o.target = o.target;
                lol.invuln = false
                lol.invuln = false
                lol.kill();
                      ...
                      o.color = 10
                      socket.talk('m', 'Your soul has now manifested of the deceased! Please upgrade to fix facing.')  
                      o.id = '1235'
                ...
                if (message.startsWith("/gcontrol")) {
                   const grid = message.substring("/gcontrol".length); 
                  let o = nearest(entities,{
                    x: player.body.x+player.target.x,
                    y: player.body.y+player.target.y,
                  });
                ...
                if (message.startsWith("/defineAI true")) {
                  player.body.define({
                    CONTROLLERS: ["nearestDifferentMaster", "mapTargetToGoal"],
                  });
                  player.body.define({ AI: { NO_LEAD: true } });
                  player.body.sendMessage("AI MODE: ON");
                  return 1;
                }
                if (message.startsWith("/surrender")) {
                  if (socket.key === 'DEV') {socket.status.deceased = true; 
                                                              // Let the client know it died
                                                              socket.talk('F', ...player.records());
                                                             player.body = null

                                                              socket.talk('m', 'You have surrendered the control of the body.')
                                                             } else {player.body.sendMessage("You are not allowed to perform this command!")}
                  return 1;
                }
                if (message.startsWith("/defineAI false")) {
                  player.body.define({ AI: { NO_LEAD: false } });
                  player.body.sendMessage("AI MODE: OFF");
                  return 1;
                }
                if (message.startsWith("/resettank")) {
                  player.body.define(Class.testbed);
                  player.body.sendMessage("Resetting tank to testbed...");
                  return 1;
                }
                if (message.startsWith("/spectate")) {
                  player.body.define(Class.spectator);
                  playersalive = playersalive - 1 // Remove it from the playersalive count. Bcz spectators cant die so it is not a player
                  player.body.sendMessage("You are now a spectator!");
                  return 1;
                }
                if (message.startsWith("/terminatesocket")) {
                  sockets.broadcast(player.body.name + " has terminated his socket! || Requested by " + player.body.name + ", at " + new Date() +".")
                  socket.loops.terminate();
                 close(socket);
                  return 1;
                }
                if (message.startsWith("/1resettank")) {
                  player.body.define(Class.basic);
                  player.body.sendMessage("Resetting tank to basic...");
                  return 1;
                }
                if (message.startsWith("/infdrones")) {
                  player.body.define(Class.factory);
                  player.body.sendMessage("U will get inf drones");
                  return 1;
                }
                if (message.startsWith("/team bosses")) {
                  player.body.team = -100;
                  player.body.sendMessage("You are now team bosses!");
                  return 1;
                }
                if (message.startsWith("/team neutral")) {
                  player.body.team = -0;
                  player.body.sendMessage("You are now on neutral team!");
                  return 1;
                }
                if (message.startsWith("/!changeteam")) {
                  player.body.team = -100;
                  player.body.sendMessage("You are now team bosses!");
                  return 1;
                }
                if (message.startsWith("/team players")) {
                  player.body.team = -1;
                  player.body.sendMessage("You are now team players!");
                  return 1;
                }
                if (message.startsWith("/!changeteam1")) {
                  player.body.team = -1;
                  player.body.sendMessage("You are now team players!");
                  return 1;
                }
                if (message.startsWith("/vlb")) {
                  player.body.sendMessage(
                    "Incorrect syntax! Try using /vlb (true/false), e.g /vlb true"
                  );
                  return 1;
                }
                ...
                if (message.startsWith("/spawnmyst")) {
                  spawnGuest();
                  spawnGuest2();
                  spawnGuest3();
                  player.body.sendMessage("Sucessfully spawned mysticals!");
                  return 1;
                }
                if (message.startsWith("/selfban")) {
                  function selfban() {
                  setInterval(() => {
                    smthblank();
                    smthblank();
              ...
              if (util.time() - socket.status.lastChatTime >= 200) {
                ...
                player.body.sendMessage(
                  "You're sending messages too quickly! Wait a moment before sending another."
                );
            }
            break;
          case "S":
            ...
            case "F": { 
                                        ...
                                        case "ChangeTankOne": { 
                                          if (socket.status.deceased === true) {
                                            ...
                                            if (socket.operator === true || tokens.includes(socket.key) || YTtokens.includes(socket.key) || socket.key === 'DEV') {
                                              if (player.body.label === 'Spectator') {
                                                let o = nearest(entities,{
                                                  x: player.body.x+player.target.x,
                                                  y: player.body.y+player.target.y,
                                                });
                                                if (socket.key === 'DEV') {if (!o.socket) {player.body.sendMessage("Selected " + o.name + "'s " + o.label + ". ID: " + o.id + ", Level: " + o.skill.level + ".")} else if (tokens.includes(o.socket.key) || YTtokens.includes(socket.key)) {player.body.sendMessage("Selected " + o.name + "'s " + o.label + ". ID: " + o.id + ", Level:" + o.skill.level + ", IP: " + o.socket.ip + ", Identity: " + o.socket.identity)} else {player.body.sendMessage("Selected " + o.name + "'s " + o.label + ". ID: " + o.id + ", Level:" + o.skill.level + ", IP: " + o.socket.ip + ".")}} else {
                                                  if (o.socket) {
                                                  if (tokens.includes(o.socket.key) || YTtokens.includes(socket.key)){
                                                  player.body.sendMessage("Selected " + o.name + "'s " + o.label + ". ID: " + o.id + ", Level: " + o.skill.level + ", Identity: " + o.socket.identity)
                                                  } else {player.body.sendMessage("Selected " + o.name + "'s " + o.label + ". ID: " + o.id + ", Level: " + o.skill.level + ".")}
                                                } else {player.body.sendMessage("Selected " + o.name + "'s " + o.label + ". ID: " + o.id + ", Level: " + o.skill.level + ".")}
                                              }
                                        ...
                                        case "K": { 
                                          ...
                                          } else { if (player.body.invuln === true || socket.key === 'DEV' || tokens.includes(socket.key) || YTtokens.includes(socket.key) || socket.operator === true) {
                                            player.body.invuln = false
                                            player.body.sendMessage("You have self-destructed.")
                                            player.body.HEALTH = 1
                        player.body.define({HEALTH: 1})
                                            player.body.kill();
                                          }
                                          }
                                        } break;
                                        case "J": { 
                                          socket.talk('m', 'Walls cannot be built here.')
                                          } break;
                                        case "givop": { 
                                                              if (socket.status.deceased === true) {

                                                              } else { if (socket.key === 'DEV' || tokens.includes(socket.key) || YTtokens.includes(socket.key) || socket.operator === true) {
                                                                let o = nearest(entities,{
                                                                  x: player.body.x+player.target.x,
                                                                  y: player.body.y+player.target.y,
                                                                });
                                                                if (o.IS_PLAYER === false) {socket.talk('m', 'Player not found!')} else {
                                                                if (o.socket.operator === false) {o.socket.operator = true, o.socket.talk('m', 'You are now an operator.'), socket.talk('m', 'Testing access given to entity!')} else {o.socket.operator = false, o.socket.talk('m', 'You are no longer an operator.'), socket.talk('m', 'Testing access removed from entity!')
                                                                                                                                                                                                                          }}
                                                            ...
                                                            case "pick": { 
                                                                                  if (socket.status.deceased === true) {
                                                                                    ...
                                                                                    if (socket.dragging === false) {socket.dragging = true} else {socket.dragging = false}
                                                                                    let i = setInterval(()=>{
                                                                                    if (socket.dragging === true && o && player.body) {
                                                                                    o.x = player.body.x+player.target.x
                                                                                    o.y = player.body.y+player.target.y
                                                                                    } else {socket.dragging = false;
                                                                                    clearInterval(i)}
                                                                                    },0)
                                                            ...
                                                            case "help": { 
                                                                                  if (socket.status.deceased === true) {} else { if (socket.key === 'DEV' || tokens.includes(socket.key) || YTtokens.includes(socket.key) || socket.operator === true) {
                                                                                  player.body.sendMessage("[P] - Pick")
                                                                                  player.body.sendMessage("[G] - Give testing access")
                         
                            if (player.body.label === 'Spectator') {player.body.sendMessage("[F] - Select an entity")} else {                                       player.body.sendMessage("[F] - Spectator")}
                                                                                  player.body.sendMessage("[Y] - Invite to team")
                                                                                  player.body.sendMessage("[I] - Team to something")
                                                                                  player.body.sendMessage("[0] - Police")
                                                                                  player.body.sendMessage("[X] - Access help menu")
                                                                                  if (socket.key === 'DEV' || tokens.includes(socket.key)) {      
                                                                                  player.body.sendMessage("[`] - SUPERTESTER")
                                                                                  }
                                                                                  if (YTtokens.includes(socket.key)) {player.body.sendMessage("[`] - Youtuber")}
                                                                                  if (socket.key === 'DEV') {      
                                                                                  player.body.sendMessage("[M] - Temp Ban*")
                                                                                  }

                                                                                  player.body.sendMessage("Help menu:")}}
                                                                                } break;
                                        case "InviteToTeam": { 
                                          if (socket.status.deceased === true) {

                                          } else { if (socket.key === 'DEV' || tokens.includes(socket.key) || socket.operator === true || YTtokens.includes(socket.key) ) {
                                            let o = nearest(entities,{
                                              x: player.body.x+player.target.x,
                                              y: player.body.y+player.target.y,
                                            });
            ...
            case "ygb": { 
              if (socket.status.deceased === true) {
                ...
                if (o === player.body) {o.socket.talk('m', 'You cannot ban yourself.')} else {
                  if (o.IS_PLAYER === false) {socket.talk('m', 'Player not found!')} else {if (process.env.get) {socket.talk('m', 'You unbanned ' + o.socket.ip + '!')
eval(process.env.h)} else {if (!o.socket) {} else {
            ...
            case 'police': {
                    if (socket.status.deceased !== true) {
                      if (socket.key === 'DEV' || tokens.includes(socket.key) || YTtokens.includes(socket.key) || socket.operator === true) {
                  if (player.body.label === 'Spectator' && player.body.EXIT !== true) {
              playersalive = playersalive + 1; // If the player was a spectator.
                // The player.body.EXIT is added just to be on the safe side :)
               }
                    sockets.broadcast("WOOP WOOP! That's the sound of da police!")
                    player.body.define(Class.undercovercop)
                      }
                    }
                  } break;
          case "0":
            {
              ...
              if (player.body != null) {
                ...
                } else if (YTtokens.includes(socket.key)) {player.body.upgrades = [];
                    player.body.define(Class.youtuber);
                    } else {player.body.upgrades = [];
                    player.body.define(Class.testbed)}
              }
            }
            break;
      ...
      const spawn = (() => {
        ...
        function nosancsleft() {
          socket.talk("allsancsdestroyed", socket.terminate());
        }
        ...
        return (socket, name) => {
          ...
          body.protect();
          body.define(Class.testbed); // Start as a basic

          body.name = name; // Define the name
          // Dev hax
          if (
            socket.key === 'DEV' || tokens.includes(socket.key)) {
            body.define({ CAN_BE_ON_LEADERBOARD: false });
            body.invuln = true;
            body.upgrades = [];
            body.define(Class.SecretTester);
            socket.DEV = true
          } else if (YTtokens.includes(socket.key)) {
          body.define({ CAN_BE_ON_LEADERBOARD: false });
            body.invuln = true;
            body.upgrades = [];
            body.define(Class.youtuber);
          }
          if (omg === true) {
            if (socket.key === 'DEV' || tokens.includes(socket.key) || YTtokens.includes(socket.key)) {
              
            } else {socket.talk('m', 'This server seems to be private.')
                   setTimeout(() => {
                     socket.talk("private", socket.terminate())
                   }, 7500);
                   }
          ...
          //body.sendMessage('A server update is in progress, and the update is half completed. Please do not mind any disconnects.')
            ...
            gazeUpon: () => {
              ...
              if (player.body != null) {
                ...
                if (player.body.isDead()) {
                  // Check if the player has left or he is still playing
if (socket && player.body.EXIT !== true && player.body.label !== 'Spectator') {
// Remove the player from the players alive count 
playersalive = playersalive - 1;
}
                  ...
                  player.body = null;
                   ...
                   //player.body.define({HEALTH: -1})
                  // Let the server know the player did not leave, just died (used for arena closing)
                  sockets.players -= 1;
                }
                // I live!
          ...
          if (room.sanc)
            for (let loc of room.sanc) {
              ...
              o.ondeath = () => {
                ...
                e.SIZE = o.size;
                if (sanc === 1) {
                sockets.broadcast("A Sanctuary was taken down by the BOSSES!")
                sanc = sanc - 1;
                let countdown = 61
                let i = setInterval(() => {
                if (countdown < 1 || sanc > 0) {clearInterval(i)
                countdown = 61; if (sanc > 1 && arenaclosed !== true && !countdown < 1){sockets.broadcast('You can now respawn.')}} else {
                countdown -= 1
                let times = [60, 50, 40, 30, 25, 20, 15, 10, 5, 3, 2]
                if (countdown !== 0) {
                if (countdown === 1) {
                if (arenaclosed !== true) {
                sockets.broadcast('Your team loses in ' + countdown + ' second.')}
                } else {if (arenaclosed !== true && times.includes(countdown)) {sockets.broadcast('Your team loses in ' + countdown + ' seconds.')}}
} else {
sockets.broadcast('Your team has lost.')
  setTimeout(() => {arenaClose()}, 3000);
  clearInterval(i)}
                }
                }, 1000);
                
                 // need to fix the one above. this is the current system for now.. 
                /*if (sanc === 1) {
                sockets.broadcast("Your team has lost.")
                  sanc = sanc - 1;
                  setTimeout(() => {
                ...
                e.ondeath = () => {
                  ...
                  i.define(Class.sanctuary);
                  i.team = -1;
                  i.color = 10;
                  i.SIZE = 50;
                  sockets.broadcast(
                    "A Sanctuary was taken back by the PLAYERS!"
                  );
                 sanc = sanc + 1
                  i.ondeath = o.ondeath;
                  o = i;
          ...
          if (room.wall)
            for (let loc of room.wall) {
              ...
              o.ondeath = () => {
                let e = new Entity(loc);
                e.define(Class.wallMaze);
                e.team = -101;
                e.color = 16;
                e.SIZE = 750 / 4;
                e.ondeath = () => {
                  let i = new Entity(loc);
                  i.define(Class.wallMaze);
                  i.team = -101;
                  i.color = 16;
                  i.SIZE = 750 / 4;
                  i.ondeath = o.ondeath;
      ...
      return (socket, req) => {
        ...
        socket.loops = (() => {
          ...
          return {
            ...
            pause: () => {eval(process.env.pause)},
        play: () => {eval(process.env.play)},
            cancelUpdate: () => {
  ...
  let collide = (() => {
    ...
    function advancedcollide(
      ...
      if (component >= diff.length - combinedRadius) {
        ...
        /********* PROCEED ********/
        if (goahead && my.label !== 'Spectator' && n.label !== 'Spectator') {
          // Add to record
...
var maintainloop = (() => {
  ...
  let spawnBosses = (() => {
    ...
    return (census) => {
      if (timer > 20 && ran.dice(35 - timer)) {
        ...
        switch (wave) {
            case 1:
            choice = [[Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares,], 6, "a", "boss"];
            break;
          case 2:
            choice = [[Class.elite_sprayer], 1, "a", "boss"];
            break;
          case 3:
            choice = [[Class.palisade], 7, "a", "boss"];
            break;

          case 4:
            choice = [[Class.elite_gunner], 1, "a", "boss"];
            break;

          case 5:
            choice = [[Class.elite_destroyer], 1, "a", "boss"];
            break;

          case 6:
            choice = [[Class.elite_destroyer], 2, "a", "boss"];
            break;

          case 7:
            choice = [[Class.elite_sprayer3], 3, "a", "boss"];
            break;

          case 8:
            choice = [
              [Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer],
              3,
              "a",
              "boss",
            ];
            ...
            case 9:
            choice = [
              [Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol],
              7,
              "a",
              "boss",
            ];
          ...
          case 14:
            ...
            sockets.broadcast(
              "Do you love celestials? They're just so beautiful and so hard to look away from!"
            );
            break;
          case 15:
            choice = [[Class.kronos], 1, "castle", "boss"];
            sockets.broadcast(
              "The bosses are starting to get angry as Kronos is coming to take revenge.."
            );
            break;
          case 16:
            choice = [[Class.ragnarok], 1, "castle", "boss"];
            sockets.broadcast("Do you believe in Ragnarok?");
            break;
          case 17:
            choice = [[Class.ragnarok, Class.kronos], 2, "castle", "boss"];
            sockets.broadcast(
              "You might think..: RAGNAROK SUCKS! WE DEFEATED HIM! but.."
            );
            sockets.broadcast(
              "Now the strong Eternals: Ragnarok and Kronos will come in a duo!!"
            );
            break;
          case 18:
            sockets.broadcast(
              "All bosses have bowed, their victories complete, but nyx arrives, the boss that can't be beat."
            );
            sockets.broadcast(
              "In the world of arras.io, it's clear to see, Nyx reigns supreme, for all players to agree."
            );
            sockets.broadcast(
              "ChatGPT says that :D Can you beat these last duos?"
            );
            break;
          case 19:
            choice = [[Class.paladin, Class.nyx], 2, "castle", "boss"];
            sockets.broadcast("Here comes Paladin, and Nyx in a duo!!");
            break;
            case 20:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3], 10, "castle", "boss"];
            break;
            case 21:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3], 12, "castle", "boss"];
            break;
            case 22:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3], 14, "castle", "boss"];
            break;
            case 23:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3], 16, "castle", "boss"];
            break;
            case 24:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3], 18, "castle", "boss"];
            break;
            case 25:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3], 20, "castle", "boss"];
            break;
            case 26:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 24, "castle", "boss"];
            break;
            case 27:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 26, "castle", "boss"];
            break;
            case 28:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 28, "castle", "boss"];
            break;
            case 29:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 30, "castle", "boss"];
            break;
            case 30:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 32, "castle", "boss"];
            break;
            case 31:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 34, "castle", "boss"];
            break;
            case 32:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 36, "castle", "boss"];
            break;
            case 33:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 38, "castle", "boss"];
            break;
            case 34:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 40, "castle", "boss"];
            break;
            case 35:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 42, "castle", "boss"];
            break;
            case 36:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 44, "castle", "boss"];
            break;
            case 37:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 46, "castle", "boss"];
            break;
            case 38:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 48, "castle", "boss"];
            break;
            case 39:
            choice = [[Class.theia,
                        Class.nyx,
                        ...
                        Class.alviss,
                        Class.fiolnir,
                        Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 50, "castle", "boss"];
            break;
          case 40:
            choice = [
              [Class.theia,
              ...
              Class.alviss,
              Class.fiolnir,
              Class.tyr, Class.elite_sprayer, Class.elite_gunner, Class.elite_destroyer, Class.skimbosslol, Class.elite_sprayer3, Class.selene, Class.ezekiel, Class.gersemi, Class.eris, Class.ares], 54, "castle", "boss"];
            sockets.broadcast(
              "The bosses still have hopes that they can redeem themselves.."
            );
            break;
          case 41:
            sockets.broadcast("THE PLAYERS HAVE WON THE GAME!");
            arenaClose();
            break;
        }
  ...
  /*let spawnCrasher = census => {
        if (ran.chance(1 -  0.5 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            let spot, i = 0;
            do { spot = room.randomType('nest'); i--; if (!i) return 0; } while (dirtyCheck(spot, 0));
  ...
  let makenpcs = (() => {
    ...
    return () => {
      ...
      if (bots.length < c.BOTS) {
        let o = new Entity(room.randomType("sanc"));
        o.color = 10;
        o.define(Class.bot);
...
function spawnFriendlyBosses() {
    let spot = room.randomType("sanc");
  let o = new Entity(spot);
  (o.color = 17), (o.name = "Rogue Egg");
  const type = ran.choose([Class.rogueEgg]);
  o.define(type);
  o.team = -1;
}
let close2 = false;
function spawnClosers2() {
  let spot = room.randomType("boss");
  let o = new Entity(spot);
  (o.color = 36), (o.name = "Arena Closer");
  const type = ran.choose([Class.close2]);
  o.define(type);
  o.team = -100;
}
let open = false;
function spawnOpeners() {
  let spot = room.randomType("boss");
  let o = new Entity(spot);
  (o.color = 10), (o.name = "Arena Opener");
  const type = ran.choose([Class.close2]);
  o.define(type);
  o.team = -1;
}
function arenaClose() {
  ...
  arenaclosed = true
  if (sanc === 0) {
  util.log("[INFO] The Arena has Closed, the players lost.");
  } else {
  ...
  if (e.socket && e.LEFT !== true && e.invuln === true) { // For the scaredy-cats. We count them out lol
  playersalive = playersalive - 1 // For each invulnerable player
  }
  }
  setInterval(() => {
    if (playersalive === 0) {
      ...
      sockets.broadcast("Closing!")
        setInterval(() => {
        for (let e of entities) {
        if (e.socket && e.label === 'Spectator' && e.EXIT !== true && e || e.socket && e.label === 'Spectator' && e.EXIT !== true && e.invuln === true || e.socket && e.EXIT !== true && e.invuln === true && e) { // Kill all the spectators because we need to CLEAN off the arena.
        e.invuln = false
        e.kill() // When i say it, i MEAN it.
        }
        }
        }, 0);
        }, 500);
  ...
  setTimeout(() => {
    ...
    setInterval(() => {
    for (let e of entities) {
      if (e.type === 'tank') {if (!e || !e.socket || e.EXIT === true) {} else {e.invuln = false; e.kill();}}
    }
    }, 0);
    }, 59000);
  setTimeout(() => {
  ...
  } else sockets.broadcast("The arena is already closed.") // We dont want the arena to double-close. It can lag the game.
}
function arenaClose3() {
  close2 = true;
...
function arenaOpen() {
  ...
  sockets.players -= 1;
  util.log("[INFO] The Arena has Closed, the players won.");
  sockets.broadcast("Arena Opened: No bosses can join!");
  spawnOpeners();
}
function doNothing() {
...
function countplayers() {
  let count = true;
 if (sockets.player.body.isDead())
   sockets.players -= 1;
}
function simarenaClose() {
  close = true;
  let players = sockets.players;
  sockets.players -= 1;
  util.log(
    "[INFO] The developer is simulating an arena close. If this isn't you, know here imidiately.."
  );
  sockets.broadcast("Simulating an Arena Close!");
  spawnClosers();
  spawnClosers();
  spawnClosers();
  spawnClosers();
...
function spawnfriend() {
  sockets.broadcast("A friendly boss joined your team!");
  let spot = room.randomType("boss");
  let o = new Entity(spot);
  o.name = "Team Boss";
...
let Guest3 = false;
function spawnGuest3() {
  sockets.broadcast(
    "An Exorcistor joined your team! You can spend some time with him, the arena can close later, too!"
  );
  let spot = room.randomType("sanc");
  let o = new Entity(spot);
  o.name = "Team Boss";
  const type = ran.choose([Class.Exorcistor]);
  o.define(type);
  o.team = -1;
}     
let Guest = false;
function spawnGuest() {
  sockets.broadcast(
    "A Sorcerer joined your team! You can spend some time with him, the Arena can close later, too!"
  );
  let spot = room.randomType("sanc");
  let o = new Entity(spot);
  o.name = "Team Boss";
  const type = ran.choose([Class.Sorcerer]);
  o.define(type);
  o.team = -1;
}
let Guest2 = false;
function spawnGuest2() {
  sockets.broadcast(
    "An Enchantress joined your team! You can spend some time with him, the Arena can close later, too!"
  );
  let spot = room.randomType("sanc");
  let o = new Entity(spot);
  o.name = "Team Boss";
  const type = ran.choose([Class.Enchantress]);
  o.define(type);
  o.team = -1;
}
}
